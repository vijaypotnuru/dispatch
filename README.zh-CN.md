<p align="center">
  <img src="docs/assets/banner.jpg" alt="Dispatch — 人类与 AI，并肩前行" width="100%">
</p>
...
[官网](https://github.com/vijaypotnuru/dispatch) · [云服务](https://github.com/vijaypotnuru/dispatch/app) · [X](https://x.com/VijayPotnuru005) · [自部署指南](SELF_HOSTING.md) · [参与贡献](CONTRIBUTING.md)
...
dispatch 将编码 Agent 变成真正的队友。像分配给同事一样分配给 Agent——它们会自主接手工作、编写代码、报告阻塞问题、更新状态。
...
<p align="center">
  <img src="docs/assets/hero-screenshot.png" alt="Dispatch 看板视图" width="800">
</p>

## 功能特性

dispatch 管理完整的 Agent 生命周期：从任务分配到执行监控再到技能复用。
...
后续可用 `brew upgrade vijaypotnuru/tap/dispatch` 更新 CLI。
...
如果没有 Homebrew，可以使用安装脚本。脚本会安装 Dispatch CLI：检测到 `brew` 时通过 Homebrew 安装，否则直接下载二进制。
...
安装好 CLI（或注册 [dispatch 云服务](https://github.com/vijaypotnuru/dispatch)）后，按以下步骤将第一个任务分配给 Agent：
...
在 dispatch Web 端打开你的工作区，进入 **设置 → 运行时（Runtimes）**，你应该能看到你的机器已作为一个活跃的 **Runtime** 出现在列表中。

> **什么是 Runtime（运行时）？** Runtime 是可以执行 Agent 任务的计算环境。它可以是你的本地机器（通过 daemon 连接），也可以是云端实例。每个 Runtime 会上报可用的 Agent CLI，dispatch 据此决定将任务路由到哪里执行。
...
**简单来说：dispatch 专为团队协作打造，让团队和 AI Agent 一起高效完成项目。**
...
参与 dispatch 代码贡献，请参阅 [贡献指南](CONTRIBUTING.md)。
