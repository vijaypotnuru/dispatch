...
// dispatch CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // dispatch://auth/callback?token=<jwt>
...
// single-worktree case still lands at "Dispatch Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Dispatch Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Dispatch Canary";
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    electronApp.setAppUserModelId(
      is.dev ? "ai.dispatch.desktop.dev" : "ai.dispatch.desktop",
    );
