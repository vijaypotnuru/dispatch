...
这一页带你走一遍 Dispatch Cloud 的端到端流程——**注册 → 装 [命令行工具](/cli) → 启动 [守护进程](/daemon-runtimes) → 创建 [智能体](/agents) → 分配第一个 [任务](/tasks)**，约 5 分钟完成。
...
到 [dispatch.dev](https://dispatch.dev) 注册账号。可以用邮箱（6 位验证码）或 Google 登录。
...
1. 把命令行工具配置成连接 Dispatch Cloud
2. 打开浏览器让你登录（和 Web 登录一样的邮箱验证码 / Google OAuth）
3. 把生成的 PAT 存到 `~/.dispatch/config.json`
4. **自动启动守护进程**——开始每 3 秒轮询任务、每 15 秒发心跳

<Callout type="info">
**用的是桌面应用？** 桌面应用启动时**自动拉起守护进程**，不需要手动跑 `dispatch setup`。见 [桌面应用](/desktop-app)。
</Callout>
...
```bash
dispatch issue create --title "给 README 加一段 ASCII 架构图"
```
...
4. 结束后把结果发回 Dispatch（状态变 `completed` 或 `failed`，根据是否自动重试）
