---
title: Architecture
description: Technical architecture of the Dispatch platform.
---

## Overview

Dispatch is a Go backend + monorepo frontend (pnpm workspaces + Turborepo) with shared packages.
...
- **Entry points** (`cmd/`): `server` (HTTP API), `dispatch` (CLI + daemon), `migrate`
