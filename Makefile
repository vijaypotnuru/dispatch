.PHONY: help makehelp dev server daemon cli dispatch build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop
...
DISPATCH_APP_URL ?= $(FRONTEND_ORIGIN)
DATABASE_URL ?= postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@localhost:$(POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable
NEXT_PUBLIC_API_URL ?= http://localhost:$(PORT)
NEXT_PUBLIC_WS_URL ?= ws://localhost:$(PORT)/ws
GOOGLE_REDIRECT_URI ?= $(FRONTEND_ORIGIN)/auth/callback
DISPATCH_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@echo "==> Pulling official Dispatch images..."
	@if ! docker compose -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${DISPATCH_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting Dispatch via Docker Compose..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo ""; \
		echo "Images: $${DISPATCH_BACKEND_IMAGE:-ghcr.io/vijaypotnuru/dispatch-backend}:$${DISPATCH_IMAGE_TAG:-latest}"; \
		echo "        $${DISPATCH_WEB_IMAGE:-ghcr.io/vijaypotnuru/dispatch-web}:$${DISPATCH_IMAGE_TAG:-latest}"; \
		echo ""; \
		echo "Log in: configure RESEND_API_KEY in .env for email codes,"; \
		echo "        or set APP_ENV=development in .env (private networks only) to enable code 888888."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install vijaypotnuru/tap/dispatch"; \
		echo "  dispatch setup self-host"; \
	else \
		echo ""; \
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building Dispatch from the current checkout..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo "Local tags: dispatch-backend:dev and dispatch-web:dev."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install vijaypotnuru/tap/dispatch"; \
		echo "  dispatch setup self-host"; \
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) dispatch DISPATCH_ARGS="daemon restart --profile local"

cli: ## Run the dispatch CLI with ARGS or DISPATCH_ARGS from source
	@$(MAKE) dispatch DISPATCH_ARGS="$(DISPATCH_ARGS)"

dispatch: ## Run the dispatch CLI entrypoint directly from the Go source tree
	cd server && go run ./cmd/dispatch $(DISPATCH_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/dispatch ./cmd/dispatch
