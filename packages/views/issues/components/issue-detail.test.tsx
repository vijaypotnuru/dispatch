...
import type { Issue, TimelineEntry } from "@dispatch/core/types";
// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @dispatch/core/hooks.tsx). vi.mock("@dispatch/core/paths") only intercepts
...
// Mock @dispatch/core/workspace/hooks
vi.mock("@dispatch/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @dispatch/core/paths — after the URL-driven workspace refactor,
...
vi.mock("@dispatch/core/paths", async () => {
  const actual = await vi.importActual<typeof import("@dispatch/core/paths")>(
    "@dispatch/core/paths",
  );
  return {
    ...actual,
...
// Mock issue config
vi.mock("@dispatch/core/issues/config", () => ({
...
// Mock realtime
vi.mock("@dispatch/core/realtime", () => ({
  useWSEvent: vi.fn(),
...
// Mock react-resizable-panels (used by @dispatch/ui/components/ui/resizable)
