...
import type { ContextAnchor } from "@dispatch/core/chat";
...
import { inboxListOptions } from "@dispatch/core/inbox/queries";
...
export function ContextAnchorButton() {
  ...
  const tooltipText = isDisabled
    ? "Nothing to share with Dispatch on this page"
    : focusMode && candidate
      ? candidate.type === "issue"
        ? `Dispatch knows you're viewing ${candidate.label} · Click to turn off`
        : `Dispatch knows you're viewing project "${candidate.label}" · Click to turn off`
      : "Let Dispatch know what you're viewing";
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            disabled={isDisabled}
            aria-label={
              focusMode ? "Stop sharing current page" : "Share current page with Dispatch"
            }
...
export function ContextAnchorCard() {
  ...
  const tooltipText =
    candidate.type === "issue"
      ? `Dispatch knows you're viewing ${candidate.label}${candidate.subtitle ? ` — ${candidate.subtitle}` : ""}`
      : `Dispatch knows you're viewing project "${candidate.label}"`;
