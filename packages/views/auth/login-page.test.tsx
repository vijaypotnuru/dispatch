...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Dispatch' title", () => {
    render(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to dispatch/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("dispatch_token", "existing-jwt");
