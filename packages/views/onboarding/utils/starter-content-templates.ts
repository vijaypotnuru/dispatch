...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how Dispatch works →](https://dispatch.dev/docs/how-dispatch-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in Dispatch**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to Dispatch — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Dispatch. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to Dispatch — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Dispatch. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to Dispatch — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Dispatch. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to Dispatch — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Dispatch. They're **exploring** what Dispatch can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to Dispatch — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Dispatch. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to Dispatch — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Dispatch.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in Dispatch are triggered when an issue has:`,
        ``,
        ...
        ``,
        `[Learn about assigning issues →](https://dispatch.dev/docs/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about chat →](https://dispatch.dev/docs/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about workspaces →](https://dispatch.dev/docs/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `Dispatch works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
        ...
        ``,
        `[Learn about members and roles →](https://dispatch.dev/docs/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[Learn about creating agents →](https://dispatch.dev/docs/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[Learn about tasks →](https://dispatch.dev/docs/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about the inbox →](https://dispatch.dev/docs/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about autopilots →](https://dispatch.dev/docs/autopilots)`,
      ].join("\n"),
    },
  ];
...
export function buildSelfServeSubIssues(
  ...
  // Without a runtime + an agent, nothing else in Dispatch works. These two
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. Go to [github.com/vijaypotnuru/dispatch/releases/latest](https://github.com/vijaypotnuru/dispatch/releases/latest) and download the \`.dmg\` for macOS`,
        ...
        ``,
        `[Learn about runtimes →](https://dispatch.dev/docs/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
...
export function buildImportPayload({
  ...
  return {
    ...
    project: {
      ...
      description:
        "A few things to try in Dispatch. Work through them at your own pace.",
      icon: "👋",
    },
