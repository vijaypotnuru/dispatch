...
This document is the source of truth for the analytics events Dispatch ships
...
See [MUL-1122](https://github.com/vijaypotnuru/dispatch) for the design context.
...
Self-hosters should **never inherit a Dispatch-issued `POSTHOG_API_KEY`** —
...
- **Person properties (`$set`)** — use for mutable cohort signals
  ...
  `setPersonProperties()` in `@dispatch/core/analytics`. Use
...
| `signup_source` | string | Opaque attribution bundle from the frontend cookie `dispatch_signup_source` (UTM + referrer). Empty when the cookie is absent. |
...
| `cli_version` | string | Version of the `dispatch` CLI that registered it. |
...
- Attribution is NOT a separate event; UTM + referrer origin are written
  to the `dispatch_signup_source` cookie on the first anonymous pageview
