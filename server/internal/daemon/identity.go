...
// same machine shares one UUID stored at `~/.dispatch/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.dispatch/profiles/*/daemon.id` and returns every
...
// explicitly pins DISPATCH_DAEMON_ID to the hostname itself, there's nothing
