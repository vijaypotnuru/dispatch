...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Dispatch Agent Runtime",
		"dispatch issue get",
		"dispatch issue comment list",
		"Go Conventions",
		"PR Review",
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Dispatch Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "dispatch issue get") {
		t.Error("should reference dispatch CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `dispatch issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"dispatch issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `dispatch issue comment add`",
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, dispatchManagedBeginMarker) || !strings.Contains(s, dispatchManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertDispatchManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), dispatchManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.dispatch.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, dispatchManagedBeginMarker)
	endIdx := strings.Index(s, dispatchManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.dispatch]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.dispatch]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.dispatch content")
	}
	...
	if n := strings.Count(string(data2), dispatchManagedBeginMarker); n != 1 {
...
` + dispatchManagedBeginMarker + `
sandbox_mode = "workspace-write"

[sandbox_workspace_write]
network_access = true
` + dispatchManagedEndMarker + `
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.dispatch], got:\n%s", s)
	}
	if strings.Count(s, dispatchManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), dispatchManagedBeginMarker) {
		t.Error("reused config.toml missing dispatch-managed block")
	}
}
