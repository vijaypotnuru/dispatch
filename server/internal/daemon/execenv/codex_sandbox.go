...
// `no such host` errors when calling out (for example, `dispatch issue get`
// hitting the Dispatch API). See upstream issue openai/codex#10390.
//
...
// actually reach the Dispatch API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Dispatch CLI can reach the API.
...
// dispatchManagedBeginMarker / dispatchManagedEndMarker delimit the block the
...
const (
	dispatchManagedBeginMarker = "# BEGIN dispatch-managed (do not edit; regenerated by daemon)"
	dispatchManagedEndMarker   = "# END dispatch-managed"
)

// renderDispatchManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.dispatch]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertDispatchManagedBlock), avoids both traps.
func renderDispatchManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(dispatchManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(dispatchManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(dispatchManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(dispatchManagedEndMarker) + `\n?`)

// upsertDispatchManagedBlock returns the config content with the dispatch-managed
...
// `[permissions.dispatch]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderDispatchManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertDispatchManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderDispatchManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the dispatch-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertDispatchManagedBlock(existing, policy)
