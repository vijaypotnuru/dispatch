...
type Config struct {
	...
	DeviceName         string
	RuntimeName        string
	CLIVersion         string                // dispatch CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot     string                // base path for execution envs (default: ~/dispatch_workspaces)
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("DISPATCH_SERVER_URL", DefaultServerURL)
	...
	claudePath := envOrDefault("DISPATCH_CLAUDE_PATH", "claude")
	if _, err := exec.LookPath(claudePath); err == nil {
		agents["claude"] = AgentEntry{
			Path:  claudePath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_CLAUDE_MODEL")),
		}
	}
	codexPath := envOrDefault("DISPATCH_CODEX_PATH", "codex")
	if _, err := exec.LookPath(codexPath); err == nil {
		agents["codex"] = AgentEntry{
			Path:  codexPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_CODEX_MODEL")),
		}
	}
	opencodePath := envOrDefault("DISPATCH_OPENCODE_PATH", "opencode")
	if _, err := exec.LookPath(opencodePath); err == nil {
		agents["opencode"] = AgentEntry{
			Path:  opencodePath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_OPENCODE_MODEL")),
		}
	}
	openclawPath := envOrDefault("DISPATCH_OPENCLAW_PATH", "openclaw")
	if _, err := exec.LookPath(openclawPath); err == nil {
		agents["openclaw"] = AgentEntry{
			Path:  openclawPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_OPENCLAW_MODEL")),
		}
	}
	hermesPath := envOrDefault("DISPATCH_HERMES_PATH", "hermes")
	if _, err := exec.LookPath(hermesPath); err == nil {
		agents["hermes"] = AgentEntry{
			Path:  hermesPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_HERMES_MODEL")),
		}
	}
	geminiPath := envOrDefault("DISPATCH_GEMINI_PATH", "gemini")
	if _, err := exec.LookPath(geminiPath); err == nil {
		agents["gemini"] = AgentEntry{
			Path:  geminiPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_GEMINI_MODEL")),
		}
	}
	piPath := envOrDefault("DISPATCH_PI_PATH", "pi")
	if _, err := exec.LookPath(piPath); err == nil {
		agents["pi"] = AgentEntry{
			Path:  piPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_PI_MODEL")),
		}
	}
	cursorPath := envOrDefault("DISPATCH_CURSOR_PATH", "cursor-agent")
	if _, err := exec.LookPath(cursorPath); err == nil {
		agents["cursor"] = AgentEntry{
			Path:  cursorPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_CURSOR_MODEL")),
		}
	}
	copilotPath := envOrDefault("DISPATCH_COPILOT_PATH", "copilot")
	if _, err := exec.LookPath(copilotPath); err == nil {
		agents["copilot"] = AgentEntry{
			Path:  copilotPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_COPILOT_MODEL")),
		}
	}
	kimiPath := envOrDefault("DISPATCH_KIMI_PATH", "kimi")
	if _, err := exec.LookPath(kimiPath); err == nil {
		agents["kimi"] = AgentEntry{
			Path:  kimiPath,
			Model: strings.TrimSpace(os.Getenv("DISPATCH_KIMI_MODEL")),
		}
	}
	if len(agents) == 0 {
	...
	pollInterval, err := durationFromEnv("DISPATCH_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("DISPATCH_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("DISPATCH_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	maxConcurrentTasks, err := intFromEnv("DISPATCH_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via DISPATCH_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("DISPATCH_DAEMON_ID"))
	...
	// — e.g. ~/.dispatch/daemon.id (default) plus ~/.dispatch/profiles/<x>/
	...
	// the user explicitly pins DISPATCH_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("DISPATCH_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("DISPATCH_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/dispatch_workspaces or ~/dispatch_workspaces_<profile>)
	workspacesRoot := strings.TrimSpace(os.Getenv("DISPATCH_WORKSPACES_ROOT"))
	...
	if workspacesRoot == "" {
		...
		if err != nil {
			return Config{}, fmt.Errorf("resolve home directory: %w (set DISPATCH_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			workspacesRoot = filepath.Join(home, "dispatch_workspaces_"+profile)
		} else {
			workspacesRoot = filepath.Join(home, "dispatch_workspaces")
		}
	}
	...
	keepEnv := os.Getenv("DISPATCH_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("DISPATCH_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("DISPATCH_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("DISPATCH_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("DISPATCH_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("DISPATCH_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid DISPATCH_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("DISPATCH_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
